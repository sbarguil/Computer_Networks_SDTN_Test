...
\shorttitle{Hybrid SDN  Multilayer Use Cases Implementation: Topology Visualization and Service Provisioning}
...
\begin{abstract}
Software-Defined Networking (SDN) is a dominant paradigm already transforming everyday operations in Telecommunications Networks and Providers. SDN came out with the idea of decoupling forwarding and control plane in campus switches via OpenFlow, but it has evolved to meet with the needs of extensive production networks. Network operator cannot change the entire network's footprint and upgrade the entire network to support SDN at once. This is why Telco operators adapt the initial SDN concept into a hybrid SDN approach to have a pragmatic, evolutionary, and economically viable solution. This paper presents and tests a Hybrid Hierarchical SDN architecture based on a set of SDN controllers for each technological domain and a Software-Defined Transport Networking (SDTN) controller, which deals with the end-to-end multilayer, multidomain aspects of the network services, topology, performance and inventory. Such a hierarchical approach allows a service provider to migrate portions of the network progressively into SDN domains, starting with abstract programmatic interfaces and, if the devices supports it, programming each device automatically with a standard interface.

Service provisioning is critical for supplying next-generation services to customers of all sizes and characteristics. Historically, humans manually perform service provisioning within a network; after this there are semi-automatic operations supported by service activator-tuned tools for the vendor-specific combination that comprised the Network. With the advent of SDN, service delivery operations can be performed in a vendor-agnostic fashion using standard data models and novel protocols. However, new challenges persist, such as orchestrating multiple layers required for covering long-haul, medium, and short distances. Multidomain networking between IP/MPLS-based layers and underlying WDM multilayer technologies require further coordination and orchestration. This paper presents the use of SDN to enable multidomain service provisioning and the corresponding end-to-end topology visualization. Moreover, this work relies on a hybrid SDN architecture combining the capabilities of a SDTN controller with Commercial off-the-shelf devices.
...
The paper is structured as follows: \cref{section:arq} describes the proposed SDN reference architecture, named iFUSION. This architecture was defined to evolve Telefonica's transport network in the coming years using the hybrid-SDN approach. \Cref{section:net} details the principles of network programmability, including the concepts of YANG modelling and protocols used within the tests. \Cref{section:models} defines and differentiates service and device models. Additionally, we describe the specific models used during the evaluation for IP/Optical service provisioning and topology collection. Based on the defined models, \cref{sec:usecasesdef} specifies the set of use cases for this implementation. \Cref{section:trial} details the test scenarios and cases (based on iFUSION premises), including SDN controllers and underlay network employed. \Cref{section:results} reports the results achieved in this Field Trial evaluation. Finally, \cref{section:conclusions} summarizes the conclusion of the present work and the next steps.
...
\Cref{FIG:l3vpn_workflow} depicts the workflow used by the SDTN to create and retrieve the services on the IP SDNc. Each creation block represents a set of  creation (POST) and retriveal (GET) operations. The six blocks of requests depicted corresponds to: 
...
The UML diagram on \Cref{FIG:optical_provisioning_workflow} shows the HTTP POST request involved in the optical circuit creation as sent from the SDTN towards the optical SDNc, as well as the HTTP GET requests for information retrieval regarding particular connectivity services existing on the network. 
...
The workflow between the SDTN and the IP controller has been summarized in the UML model presented on \cref{FIG:l3vpn_workflow}. \Cref{FIG:l3vpn_results} shows the three visualization options provided by the SDTN, each pane has the following information:
\begin{enumerate}
...
In the IP domain, the network topology implementations done by the IP SDNc's providers just included the network side ports. Thus, the possible service end-points (i.e access ports available) were not able to be collected automatically from the SDTN. For that reason, the interfaces and ports available were added manually to the SDTN inventory.
...
This use case has been successfully tested using a single IP controller using the L2SM model to request service creation between the SDTN and the IP SDNc. \Cref{FIG:L2SM_results} shows the L2VPN service creation results as seen from the SDTN GUI. The results have three information panes including the service details: 
\begin{itemize}
...
\Cref{FIG:optical_provisioning_result} depicst a multi-layer view of a ten GbE (10GbE) service created in one of the optical controllers.
