...
\begin{enumerate}
    ...
    \item Service Creation. VC-ID to be negotiated by the ends is provided.  
...
\begin{figure}
	\centering
		\includegraphics[width=\linewidth]{figs/l2sm_workflow.png}
	\caption{L2SM workflow}
	\label{L2SM_workflow}
\end{figure}

The workflow between the SDTN and the IP Controller has been summarized in the UML model presented on \ref{FIG:L2SM_workflow}, in which not only the POST requests have been included but also the GET requests sent towards the controller for information retrieval purposes. 
